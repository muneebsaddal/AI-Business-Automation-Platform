...
export default function ExecutionTrace({ events = [] }) {
  ...
  return (
    <section className="border border-line bg-ink p-5 text-paper shadow-panel">
      ...
      <div className="mt-4 max-h-[420px] space-y-3 overflow-auto pr-2">
        ...
        {events.map((event, index) => (
          <article key={`${event.event}-${index}`} className="border border-white/10 bg-white/5 p-3">
            <div className="flex items-start justify-between gap-3">
              <p className="text-sm font-semibold">{eventTitle(event)}</p>
              <span className="shrink-0 text-xs text-paper/50">{index + 1}</span>
            </div>
            <pre className="mt-2 overflow-auto whitespace-pre-wrap text-xs leading-5 text-paper/65">
